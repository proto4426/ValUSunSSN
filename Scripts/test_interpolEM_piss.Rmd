...
```{r}
res_all <-  (filledt.vero$UC2) - (zssn[,"wnUC2"][1:nrow(df_uccle_vero)])
...
res_fil <-  (filledt.vero$UC2[zssn_complete$miss_wnUC2]) - filled_ssn
...
filledt.chris <- read.csv("/home/piss/PissoortRepo/ValUSunSSN/data/SSNfilledChris.csv") # Load data

load("/home/piss/PissoortRepo/ValUSunSSN/Data/DataSSN60.RData") # Load data we have build
...
## Residuals
res_all <- (filled.chris2$wnUC2_d.txt) - (zssn[,"wnUC2"][1:nrow(filled.chris2)])
...
scale_colour_gradientn(colours = myPalette(10),limits=c(-50, 50))
...
res_fil <- (filled.chris2$wnUC2_d.txt[zssn_complete$miss_wnUC2]) - filled_ssn 
...
```{r, eval = F}
library(softImpute)
load('/home/piss/PissoortRepo/ValUSunSSN/data/dataSSN_all.RData')   # Take full dataset
